...
function getMediaDir(envKey: string, defaultSubPath: string): string {
  const envValue = process.env[envKey];
  if (envValue) {
    return envValue;
  }
  if (process.env.NODE_ENV === 'production') {
    return path.join('/app', 'public', 'media', defaultSubPath);
  }
  return path.join(baseDir, 'public', 'media', defaultSubPath);
}
...
export function getConfig(): Config {
  ...
  return {
    ...
    log_file: getEnv('LOG_FILE', path.join(baseDir, 'logs', 'all.log')),
...
export function validateConfig(config: Config): { valid: boolean; warnings: string[] } {
